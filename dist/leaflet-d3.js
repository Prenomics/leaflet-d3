...
(function (global, factory) {
	typeof exports === 'object' && typeof module !== 'undefined' ? factory(require('d3-hexbin'), require('d3-selection'), require('d3-dispatch'), require('d3-scale'), require('d3-array'), require('leaflet')) :
	typeof define === 'function' && define.amd ? define(['d3-hexbin', 'd3-selection', 'd3-dispatch', 'd3-scale', 'd3-array', 'leaflet'], factory) :
	(global = global || self, factory(global.d3.hexbin, global.d3Selection, global.d3Dispatch, global.d3Scale, global.d3Array));
}(this, (function (d3Hexbin, d3Selection, d3Dispatch, d3Scale, d3Array) { 'use strict';
	...
	L.HexbinLayer = L.SVG.extend({
		...
		initialize: function (options) {
			...
			this._scale = {
				...
				radius: d3Scale.scaleLinear()
			};

			// Set up the Dispatcher for managing events and callbacks
			this._dispatch = d3Dispatch.dispatch('mouseover', 'mouseout', 'click');
		...
		onRemove: function (map) {
			...
			d3Selection.select('g.hexbin').remove();
		...
		_initContainer: function () {
			...
			this._d3Container = d3Selection.select(this._container).select('g');
		},
		...
		_getExtent: function (bins, valueFn, scaleExtent) {
			...
			var _extent = d3Array.extent(bins, valueFn.bind(this));
	...
	L.HexbinHoverHandler = {
		...
		tooltip: function (options) {
			...
			var tooltip = d3Selection.select('.map.leaflet-container')
				.append('div')
			...
			return {
				mouseover: function (hexLayer, data) {
					var gCoords = d3Selection.mouse(this);
					...
					tooltip
						.style('top', '' + (d3Selection.event.clientY - gCoords[1] - h - 16) + 'px')
						.style('left', '' + (d3Selection.event.clientX - gCoords[0] - w / 2) + 'px');
		...
		resizeFill: function () {
			...
			return {
				mouseover: function (hexLayer, data) {
					var o = d3Selection.select(this.parentNode);
